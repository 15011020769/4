  ...
  <div class="menu-div">
    <ul class="ul-1">
      <li v-for="(item, index1) in expanders" class="li-1" :key="index1">
        <a @click="navFunc1(item.title)" href="javascript:void(0);" class="a-1"
          v-class="{true:'long-9999',false:'long-0000'}[navActionA===item.title]">
          {{ item.title }}
        ...
        <ul v-show="navActionC === item.title" class="ul-2">
          <li v-for="(children, index2) in item.text" :key="index2" @click="navFunc2(children.title)" class="li-2"
            :class="activeMenu === children.route ? 'active' : ''">
            <a v-if="children.href" :href="children.href" class="a-2">{{ children.title }}</a>
            <a v-else @click="activeMenu=children.route;$router.push(children.route);" href="javascript:void(0);"
              class="a-2">{{ children.title }}</a>
          </li>
        </ul>
      </li>
    </ul>
