    ...
    <div class="btn-style">
      <el-row>
        <el-button-group>
          ...
          <el-button size="small" @click="TimeChoice(1*24*2)" :type="classsvalue == 1*24*2 ? 'primary' : ''">前天
          </el-button>
    ...
    methods: {
      ...
      TimeChoice(time) {
        ...
        if (time === 1) {
          ...
          const KTime = moment(new Date()).subtract(2,'hours').format("YYYY-MM-DD HH:mm:ss"); //获取当前时间
          ...
          // 
          const noeago = moment(startTime - 86400000).subtract(2,'hours').format(
            "YYYY-MM-DD HH:mm:ss"
          ); //获取1天前的时间
          ...
          const KTime = moment(new Date()).format("YYYY/MM/DD HH:mm:ss"); //获取当前时间
          const startTime = new Date(KTime).getTime();
          const zhouago = moment(startTime - 172800000).subtract(2,'hours').format(
            "YYYY-MM-DD HH:mm:ss"
          ); //获取1周前的时间
          const zhounow = moment(startTime - 172800000).format(
            "YYYY-MM-DD HH:mm:ss"
          );
