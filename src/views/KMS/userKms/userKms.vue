<template>
  <div>
    ...
    <div class="mainContent">
      <KMSdialog :dialogVisibleKMS="dialogVisibleKMS" @_confirm="_confirm" @_cancel="_cancel" :KMStitle="KMStitle"
        :KMStxt="KMStxt" :KMSdata="KMSdata" :state="state" />
      <div class="mainContBtn newClear">
        ...
        <el-dialog class="dialogModel" :title="$t('KMS.total.newCreateKms')" :visible.sync="dialogVisible" width="50%"
        ...
        <div class="conRightSearch">
          <el-input class="iptSearch" :placeholder="$t('KMS.total.placeholder1')" v-model="tableDataName"
            @change="inpVal"></el-input>
        ...
        <div class="tableList">
          <el-table ref="multipleTable" :data="tableDataBegin.slice((currpage - 1) * pagesize, currpage * pagesize)"
            ...
            <el-table-column prop="KeyState" :label="$t('KMS.total.status')">
              <template slot-scope="scope">
                <span
                  :style="scope.row.KeyState=='Disabled'||scope.row.KeyState=='PendingDelete'||scope.row.KeyState=='PendingImport'?'color:#ff9d00':'color:#000'">{{scope.row.KeyState=="PendingDelete"?'於'+timestampToTime(scope.row.DeletionDate)+'徹底刪除':filterState(scope.row.KeyState)}}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="kmsChange" :label="$t('KMS.total.kmsChange')">
              <template slot-scope="scope">
                <a href="#" class="aColorGree"
                  ...
                  @click="startChange(scope.row,$event)">{{$t('KMS.total.startChange')}}</a>
                ...
                <a href="#" class="aColorGree"
                  ...
                  @click="startChange(scope.row,$event)">{{$t('KMS.total.stopChange')}}</a>
              </template>
            </el-table-column>
            <el-table-column prop="action" :label="$t('KMS.total.action')" class="action">
              <template slot-scope="scope">
                <a href="#" class="aColorGree"
                  ...
                  @click="startKms(scope.row,$event)">{{$t('KMS.total.startKms')}}</a>
                <span class="spanLine">|</span>
                <a href="#" class="aColorGree"
                  ...
                  @click="startKms(scope.row,$event)">{{$t('KMS.total.stopKms')}}</a>
                <br />
                <a href="#" class="aColorGree"
                  :style="scope.row.KeyState=='PendingDelete'?'pointer-events:none':'color:#006eff'"
                  @click="openDelete(scope.row,$event)">{{$t('KMS.total.planDelete')}}</a>
                <span class="spanLine">|</span>
                <a href="#" class="aColorGree"
                  :style="scope.row.KeyState=='PendingDelete'?'color:#006eff':'pointer-events:none'"
                  @click="openDelete(scope.row,$event)">{{$t('KMS.total.closeDelete')}}</a>
              </template>
            </el-table-column>
          </el-table>
          <stopChange :isShow="dialogModelChange" :content="doalogModelBox" @parentByClick="childrenShow"
            @startSure="startSure" @stopSure="stopSure" />
          <startKms :isShow="dialogModelKms" :content="doalogModelBox1" @parentByClick="childrenShow1"
            @startKmsSure="startKmsSure" @stopKmsSure="stopKmsSure" />
          <openDelete :isShow="dialogModelDelete" :content="doalogModelBox2" @parentByClick="childrenShow2"
            @openDeleteSure="openDeleteSure" @closeDeleteSure="closeDeleteSure" />
        </div>
        <el-dialog class="dialogModel" :title="$t('KMS.total.planDeleteKms')" :visible.sync="dialogModelOpenDelete"
          width="40%" :before-close="handleCloseOpenDelete">
        ...
        this.axios.post(NEW_KMS, params).then(res => {
          if (res.Response.Error !== undefined) {
            let ErrTips = {
              ...
              'InvalidParameterValue.InvalidKeyUsage': 'KeyUsage參數錯誤',
              'InvalidParameterValue.InvalidType': 'Type參數錯誤',
              'LimitExceeded.CmkLimitExceeded': 'CMK數量已達上限',
              'UnauthorizedOperation': '未授權操作',
            };
            let ErrOr = Object.assign(ErrorTips, ErrTips);
            this.$message({
            ...
            this.createForm.discription = ''
            this.createForm.Type = 'KMS'
            this.getData();
          }
        });
      },
