        ...
        <div class="tableList">
          <el-table ref="multipleTable" :data="tableDataBegin.slice((currentPage-1)*pageSize,currentPage*pageSize)" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="kmsChange" label="密钥轮换">
              <template slot-scope="scope">
                <a href="#" class="aColorGree" :style="scope.row.KeyState=='PendingDelete'||scope.row.KeyState=='PendingImport'||scope.row.KeyRotationEnabled==true?'pointer-events:none':'color:#006eff'" @click="startChange(scope.row,$event)">启用轮换</a>
            ...
            <el-table-column prop="action" label="操作" class="action">
              <template slot-scope="scope">
                ...
                <span class="spanLine">|</span>
                <a href="#" class="aColorGree" :style="scope.row.KeyState=='Disabled'||scope.row.KeyState=='PendingDelete'||scope.row.KeyState=='PendingImport'?'pointer-events:none':'color:#006eff'" @click="startKms(scope.row,$event)">禁用密钥</a><br />
