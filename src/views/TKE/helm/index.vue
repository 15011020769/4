<template>
  <div class="helm-wrap">
    ...
    <div class="wrap">
      ...
      <div class="room-center" v-show="flagSE">
        <div class="explain" v-show="flagAgin == 1 ? true:false">
          <p>
            {{$t('TKE.overview.wktxaz')}}
            <a
              @click="centerDialogVisible3 = true"
              style="cursor:pointer"
            >{{$t('TKE.overview.qskt')}}</a>
          </p>
        </div>
      ...
      <div class="room-center" v-show="getflags">
        <div class="explain-o">
          <p>
            {{$t('TKE.overview.cjxyy')}}
            <i class="el-icon-loading"></i>
            <a
              style="cursor:pointer"
              @click="centerDialogVisible4 = true"
            >{{$t('TKE.overview.ckxq')}}</a>
          </p>
        </div>
      </div>
            ...
            <el-table-column :label="$t('TKE.overview.yym')" max-width="10%" prop="name">
              <template slot-scope="scope">
                <div>
                  ...
                  <a style="cursor:pointer" @click="jumpDetail(scope.row)">{{scope.row.name}}</a>
                </div>
              </template>
            </el-table-column>
    ...
    <!-- 更新helm应用弹窗 -->
    <el-dialog :title="$t('TKE.overview.gxhelm')" :visible.sync="centerDialogVisible" width="45%">
      <div>
        <div class="explain" style="margin-bottom:20px;">
          <p>{{$t('TKE.overview.zyi')}}</p>
        </div>
        <el-form
          class="tke-form m0"
          :model="ruleForm"
          ...
          size="mini"
        >
          <el-form-item :label="$t('TKE.overview.yym')">{{ruleForm.name}}</el-form-item>
          <el-form-item :label="$t('TKE.overview.chartmc')">{{ruleForm.chart}}</el-form-item>
    ...
    <!-- 删除 -->
    <el-dialog :title="'您確定要刪除嗎'+ruleForm.name" :visible.sync="centerDialogVisible2" width="40%">
    ...
    >
      <div>
        ...
        <p>
          {{$t('TKE.overview.zyjq')}}
          <span style="color: #ff9d00">0.28核 CPU 180Mi {{$t('TKE.overview.ncun')}}</span>
          的{{$t('TKE.overview.zy')}}
        </p>
      </div>
      ...
      <div style="width:100%;height:400px;">
        <div style="width:40%;height:400px;float:left;">
          ...
          >
            ...
            <el-table-column :label="$t('TKE.overview.helmmc')" prop="name">
            ...
            <el-table-column fixed="right" label="操作">
              <template slot-scope="scope">
                <el-button
                  @click="deleteStatus(scope)"
                  type="text"
                  size="small"
                >{{$t('TKE.overview.qxaz')}}</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
        <div style="width:60%;height:400px;background:black;float:left;color:white;">{{tableError}}</div>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    getHelmList() {
      const param = {
        ...
        Path:
          ...
          this.value +
          "/helm/tiller/v2/releases/json?status_codes=DEPLOYED&&status_codes=FAILED&&status_codes=DELETING&&status_codes=DELETED&&status_codes=UNKNOWN&&sort_by=LAST_RELEASED&&sort_order=DESC&&limit=10",
        Version: "2018-05-25"
      };
    ...
    setHelm() {
      ...
      if (
        ...
        var RequestBodyAll = {
          chart_url: this.ruleForm.address,
          username: this.ruleForm.nameTwo,
    ...
    getPost() {
      const param = {
        ...
        RequestBody: {
          kind: "Helm",
          apiVersion: "platform.tke/v1",
          metadata: { generateName: "hm" },
          spec: { clusterName: this.value }
        },
        Version: "2018-05-25"
      };
      this.axios.post(POINT_REQUEST, param).then(res => {
    ...
    deleteStatus(scope) {
      ...
      const param = {
        ...
        Path:
          "/apis/platform.tke/v1/clusters/" +
          this.value +
          "/helm/tiller/v2/releases/installing/" +
          scope.row.name +
          "/json",
        Version: "2018-05-25"
      };
