  ...
  <div class="room">
    <div class="room-padding" v-if="this.Data&&this.info">
      ...
      <el-card class="top">
        ...
        <el-table style="width: 100%" class="top" :data="tableDate">
          <el-table-column prop="resource" :label="$t('TKE.overview.zy')" max-width="33%">
          ...
          <el-table-column label="操作" max-width="34%" prop="yaml">
            <template slot-scope="scope">
              <a
                style="cursor:pointer;"
                @click="getYaml(scope.row.yaml)"
              >{{$t('TKE.overview.ckyaml')}}</a>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
    ...
    <el-dialog :title="$t('TKE.overview.ckyaml')" :visible.sync="centerDialogVisible" width="60%">
      <div class="yaml">
        <codemirror
