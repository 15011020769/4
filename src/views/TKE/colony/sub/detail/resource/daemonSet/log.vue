  ...
  <div class="colony-main">
    <div class="tke-card mt10 tke-formpanel-wrap">
      ...
      <el-card class='box-card'> 
        ...
        <el-select v-model="currDaemonSet" @change="changeContainer">
          <el-option
            v-for="(item,i) in this.daemonSetList"
            :key="i"
            :label="item.name"
            :value="item.name"
            >
...
export default {
  ...
  data() {
    return {
      ...
      index: 100,//选中的条数
      daemonSetList: [],//statefulSet列表
      potList:[],//pod列表
      currPod: '',//选中的pod
      currDaemonSet: '',//选中的stateful
...
        autoCloseBrackets: true
  ...
  methods: {
    async getdaemonSetLog() {
      ...
      let param = {
        Method: "GET",
        Path: "/api/v1/namespaces/"+this.rowData.metadata.namespace+"/pods/"+this.currPod+"/log?container="+this.currDaemonSet+"&timestamps=true&tailLines="+this.index,
        Version: "2018-05-25",
