 ...
 <template>
   <div>
     ...
     <div class="tke-grid ">
       <!-- 左侧 -->
       <div class="grid-left">
         <el-button @click="goPvcCreate()" size="small" type="primary">新建</el-button>
       </div>
       <!-- 右侧 -->
       <div class="grid-right" v-if="searchType">
         <tkeSearch typeSelect refreshData exportData typeLabel='命名空间' :typeOptions='searchOptions'
           :typeValue='searchType' inputPlaceholder='请输入关键词搜索' :searchInput='searchInput' @changeType="changeSearchType"
           @changeInput="changeSearchInput" @clickSearch="clickSearch" @refresh='refreshList'
   ...
   export default {
     ...
     methods: {
       ...
       getList() {
         var params = {
           Method: "GET",
           Path: "/api/v1/namespaces/" + this.searchType + "/persistentvolumeclaims?limit="+this.pageSize,
           Version: "2018-05-25",
           ClusterName: this.$route.query.clusterId
         };
         this.axios.post(POINT_REQUEST, params).then(res => {
       ...
       SearchList() {
         var params = {
            ClusterName: this.$route.query.clusterId,
            Method: "GET",
            Path: "/api/v1/namespaces/"+this.searchType+"/persistentvolumeclaims?fieldSelector=metadata.name="+this.searchInput,
            Version: "2018-05-25"
         };
         this.axios.post(POINT_REQUEST, params).then(res => {
           if (res.Response.Error === undefined) {
