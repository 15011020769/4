        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span @click="goStatefulSetDetail(scope.row)" class="tke-text-link">{{scope.row.metadata && scope.row.metadata.name}}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop=""
  ...
  methods: {
      ...
      await this.axios.post(POINT_REQUEST, param).then(async (res) => {
        if(res.Response.Error === undefined) {
          ...
          if(this.searchInput === '') {
            params = {
              Method: "GET",
              Path: "/apis/apps/v1beta2/namespaces/"+nameSpace.metadata.name+"/statefulsets?limit="+this.pageSize,
              Version: "2018-05-25",
              ClusterName: this.clusterId
            }
          } else {
            params = {
              Method: "GET",
              Path: "/apis/apps/v1beta2/namespaces/"+nameSpace.metadata.name+"/statefulsets?fieldSelector=metadata.name="+this.searchInput,
              Version: "2018-05-25",
              ClusterName: this.clusterId
            }
          }
    ...
    async getStatefulSetList() {
      ...
      if(this.searchInput === '') {
        params = {
          Method: "GET",
          Path: "/apis/apps/v1beta2/namespaces/"+this.searchType+"/statefulsets?limit="+this.pageSize,
          Version: "2018-05-25",
          ClusterName: this.clusterId
        }
      } else {
        params = {
          Method: "GET",
          Path: "/apis/apps/v1beta2/namespaces/"+this.searchType+"/statefulsets?fieldSelector=metadata.name="+this.searchInput,
          Version: "2018-05-25",
          ClusterName: this.clusterId
        }
      }
