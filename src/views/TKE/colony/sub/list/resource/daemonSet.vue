        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span class="tke-text-link" @click="goDaemonSetDetail(scope.row)">{{scope.row.metadata && scope.row.metadata.name}}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop=""
  ...
  methods: {
      ...
      await this.axios.post(POINT_REQUEST, param).then(async (res) => {
        if(res.Response.Error === undefined) {
          ...
          if(this.searchInput === '') {
            params = {
              Method: "GET",
              Path: "/apis/apps/v1beta2/namespaces/"+nameSpace.metadata.name+"/daemonsets?limit="+this.pageSize,
              Version: "2018-05-25",
              ClusterName: this.clusterId
            }
          } else {
            params = {
              Method: "GET",
              Path: "/apis/apps/v1beta2/namespaces/"+nameSpace.metadata.name+"/daemonsets?fieldSelector=metadata.name="+this.searchInput,
              Version: "2018-05-25",
              ClusterName: this.clusterId
            }
          }
    ...
    async getDaemonSetList() {
      ...
      if(this.searchInput === '') {
        params = {
          Method: "GET",
          Path: "/apis/apps/v1beta2/namespaces/"+this.searchType+"/daemonsets?limit="+this.pageSize,
          Version: "2018-05-25",
          ClusterName: this.clusterId
        }
      } else {
        params = {
          Method: "GET",
          Path: "/apis/apps/v1beta2/namespaces/"+this.searchType+"/daemonsets?fieldSelector=metadata.name="+this.searchInput,
          Version: "2018-05-25",
          ClusterName: this.clusterId
        }
      }
