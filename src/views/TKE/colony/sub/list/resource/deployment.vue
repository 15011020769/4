          ...
          <template slot-scope="scope">
            ...
            <el-dropdown class=" tke-dropdown" >
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="a"><span class="tke-text-link" @click="redeployment(scope.row)">重新部署</span></el-dropdown-item>
                ...
                <el-dropdown-item command="c"><span class="tke-text-link" @click="deleteDeployment(scope.row)">删除</span></el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        </el-table-column>
  ...
  methods: {
      ...
      await this.axios.post(POINT_REQUEST, param).then(async (res) => {
        if(res.Response.Error === undefined) {
          ...
          if(this.searchInput === '') {
            params = {
              Method: "GET",
              Path: "/apis/apps/v1beta2/namespaces/"+nameSpace.metadata.name+"/deployments?limit="+this.pageSize,
              Version: "2018-05-25",
              ClusterName: this.clusterId
            }
          } else {
            params = {
              Method: "GET",
              Path: "/apis/apps/v1beta2/namespaces/"+nameSpace.metadata.name+"/deployments?fieldSelector=metadata.name="+this.searchInput,
              Version: "2018-05-25",
              ClusterName: this.clusterId
            }
          }
    ...
    async getDeploymentList() {
      ...
      if(this.searchInput === '') {
        ...
        params = {
          Method: "GET",
          Path: "/apis/apps/v1beta2/namespaces/"+this.nameSpaceName+"/deployments?fieldSelector=metadata.name="+this.searchInput,
          Version: "2018-05-25",
          ClusterName: this.clusterId
        }
      }
    ...
    async submitRedeployment() {
      ...
      let params = {
        Method: "PATCH",
        Path: "/apis/apps/v1beta2/namespaces/"+this.nameSpaceName+"/deployments/"+this.deploymentName,
        Version: "2018-05-25",
        RequestBody: JSON.stringify({spec: {template:{metadata:{labels:{"qcloud-redeploy-timestamp": time}}}}}),
