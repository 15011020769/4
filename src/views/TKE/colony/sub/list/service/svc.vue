...
<template>
  <div>
    ...
    <div class="tke-grid">
      ...
      <div class="grid-right">
        <div>
          ...
          <el-select size="small" v-model="nameSpaceName" :placeholder="$t('TKE.overview.qxz')"
            @change="changNameSpace">
            <el-option v-for="item in searchOptions" :key="item.metadata.name" :value="item.metadata.name"></el-option>
          </el-select>
        </div>
        <tkeSearch refreshData exportData :typeValue="searchType" :inputPlaceholder="$t('TKE.subList.qsrgjzss')"
          :searchInput="searchInput" @changeInput="changeSearchInput" @clickSearch="clickSearch" @refresh="refreshList"
          @exportExcel="exportExcel"></tkeSearch>
      </div>
    </div>
    <!-- 数据列表展示 -->
    <div class="tke-card mt10">
      <el-table id="exportTable" :data="list" v-loading="loadShow" style="width: 100%">
        <el-table-column :label="$t('TKE.overview.mc')">
          <template slot-scope="scope">
            <span @click="goSvcDetail(scope.row.metadata.name)"
              class="tke-text-link">{{scope.row.metadata && scope.row.metadata.name}}</span>
          </template>
        </el-table-column>
        <el-table-column prop :label="$t('TKE.overview.lx')">
          <template slot-scope="scope">
            ...
            <!-- v-loading="!(scope.row.metadata.annotations['service.kubernetes.io/loadbalance-id'])?true:false" -->
              <a v-if="scope.row.metadata.annotations" target="_self"
                :href="'../CLB/index.html#/LB/LB-detail-applied/' + scope.row.metadata.annotations['service.kubernetes.io/loadbalance-id'] + '/1'"
                :class="scope.row.idDb?'tke-text-link':''">{{scope.row.metadata.annotations['service.kubernetes.io/loadbalance-id']}}</a>
              <p v-if="scope.row.idDb?true:false" style="padding-top:10px">{{$t('TKE.subList.fzjh')}}</p>
            </div>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button size="small" type="text" :disabled="scope.row.isDisabled"
              @click="scope.row.isDisabled?'':goSvcUpdteType(scope.row)">{{$t('TKE.subList.gxfwfs')}}</el-button>
            <el-button size="small" type="text" :disabled="scope.row.isDisabled"
              @click="scope.row.isDisabled?'':goEdit(scope.row)">{{$t('TKE.overview.bj')}}YAML</el-button>
            <el-button size="small" type="text" :disabled="scope.row.isDisabled"
              @click="scope.row.isDisabled?'':deleteInfo(scope.row)">{{$t('TKE.overview.sc')}}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
      <div class="tke-page">
        <div class="block">
          <el-pagination :pager-count="7" @size-change="handleSizeChange" @current-change="handleCurrentChange"
  ...
  export default {
    ...
    methods: {
      ...
      async getList() {
        ...
        if (this.searchInput !== '') {
          param.Path =
            `/api/v1/namespaces/${this.nameSpaceName}/services?fieldSelector=metadata.name=${this.searchInput}`
        }
        await this.axios.post(POINT_REQUEST, param).then(res => {
          if (res.Response.Error === undefined) {
            this.loadShow = false
