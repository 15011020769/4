...
<template>
  <div>
    ...
    <div class="tke-grid ">
      <!-- 左侧 -->
      <div class="grid-left">
        <el-button size="small" @click="goExpand" type="primary">{{$t('TKE.colony.xjjd')}}</el-button>
        ...
        <el-button size="small" :disabled="this.multipleSelection.length > 0 ? false : true" @click="showDeleteModal()">
          移出</el-button>
        <el-button size="small" :disabled="
            this.multipleSelection.length > 0 && blockadeType ? false : true
          " @click="showBlockModal()">{{$t('TKE.subList.fsuo')}}</el-button>
        <el-button size="small" :disabled="
            this.multipleSelection.length > 0 && unBlockadeType ? false : true
          " @click="showUnBlockModal()">{{$t('TKE.subList.jcfs')}}</el-button>
      </div>
      <!-- 右侧 -->
      <div class="tool">
        <div class="searchRight">
          ...
          <span>
            <el-tooltip class="tooltip" effect="dark" :content="$t('TKE.subList.dcbg')" placement="top">
    ...
    <div class="tke-card mt10">
      <el-table @selection-change="handleSelectionChange" :data="list" v-loading="loadShow" style="width: 100%"
        id="exportTable">
        ...
        <el-table-column :label="$t('TKE.colony.idjdm')">
          <template slot-scope="scope">
            ...
            <a :href="'../CVM/index.html#/cloudHost/cloudHostDetail/'+scope.row.InstanceId+'/1'" target="_self">
                ...
                <el-dropdown-item command="a">
                  <el-button :disabled="
                      ...
                      scope.row.unschedulable
                        ? true
                        : false
                    " class="tke-text-link" type="text" @click="showBlockModal(scope.row)">{{$t('TKE.subList.fsuo')}}
                  </el-button>
                </el-dropdown-item>
                <el-dropdown-item command="a">
                  <el-button :disabled="
                      ...
                      !scope.row.unschedulable
                        ? true
                        : false
                    " class="tke-text-link" type="text" @click="showUnBlockModal(scope.row)">
                ...
                <el-dropdown-item command="b">
                  <el-button :disabled="
                      ...
                      scope.row.status === 'upgrading'
                        ? true
                        : false
                    " class="tke-text-link" type="text" @click="showExpelDialog(scope.row)">{{$t('TKE.subList.qzhu')}}
                  </el-button>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
    ...
    <el-dialog :title="$t('TKE.subList.qdfs')" :visible.sync="showBlockademodal" width="35%">
      <div>
        {{$t('TKE.colony.yxze')}}<span
          style="color:#ff9d00;">{{ this.multipleSelection.length }}{{$t('TKE.overview.ge')}}</span>{{$t('TKE.overview.jd')}},<a
          @click="show = !show" style="cursor: pointer;">{{$t('TKE.overview.ckxq')}}</a>
      </div>
      <el-collapse-transition>
        <div v-show="show">
          <el-table :data="multipleSelection" height="200">
            <el-table-column property="InstanceId" :label="$t('TKE.subList.mcheng')" width="150"></el-table-column>
            <el-table-column property="InstanceName" :label="$t('TKE.colony.jdmc')" width="200"></el-table-column>
    ...
    <el-dialog :title="$t('TKE.subList.qxfs')" :visible.sync="showUnBlockademodal" width="35%">
      <div>
        {{$t('TKE.colony.yxze')}}<span
          style="color:#ff9d00;">{{ this.multipleSelection.length }}{{$t('TKE.overview.ge')}}</span>{{$t('TKE.overview.jd')}},<a
          @click="show = !show" style="cursor: pointer;">{{$t('TKE.overview.ckxq')}}</a>
    ...
    <el-dialog :title="$t('TKE.subList.qdscyxjd')" :visible.sync="showDelModal" width="35%">
      <div>
        {{$t('TKE.colony.yxze')}}<span
          style="color:#ff9d00;">{{ this.multipleSelection.length }}{{$t('TKE.overview.ge')}}</span>{{$t('TKE.overview.jd')}},<a
          @click="show = !show" style="cursor: pointer;">{{$t('TKE.overview.ckxq')}}</a>
    ...
    <el-dialog :title="$t('TKE.subList.bslx')" :visible.sync="showExpelModal" width="35%">
      <div>
        {{$t('TKE.colony.yxze')}}<span
          style="color:#ff9d00;">{{ this.podList.length || 0 }}{{$t('TKE.overview.ge')}}</span>{{$t('TKE.subList.sli')}},<a
          @click="show = !show" style="cursor: pointer;">{{$t('TKE.overview.ckxq')}}</a>
      </div>
      <el-collapse-transition>
        <div v-show="show">
          <el-table :data="podList" height="200">
            <el-table-column property="Name" :label="$t('TKE.subList.slpmc')" width="200"></el-table-column>
            <el-table-column property="Namespace" :label="$t('TKE.subList.ssjqkj')" width="200"></el-table-column>
          </el-table>
        </div>
      </el-collapse-transition>
      <p>
            ...
            if (nodeRes.Response.Error === undefined) {
              ...
              if (nodeRes.Response.InstanceSet.length > 0) {
                nodeRes.Response.InstanceSet.map(node => {
                  ...
                  if (new Date(now).getTime() - new Date(moment(node.CreatedTime).format("YYYY-MM-DD HH:mm:ss"))
                    .getTime() > 0) {
                    node.isDelete = false;
                  } else {
                    node.isDelete = true;
                  ...
                  k8sNodeList.map(k8s => {
                    ...
                    if (
                      ...
                      node.cpuTotal = (Number(k8s.status.allocatable.cpu.substring(0, k8s.status.allocatable
                        .cpu.length - 1)) / 1000).toFixed(2);
                      node.memoyTotal = (Number(k8s.status.allocatable.memory.substring(0, k8s.status
                        .allocatable.memory.length - 2)) / (1024 * 1024)).toFixed(2);
