    ...
    <div class="colony-main">
      ...
      <div class="tke-card tke-formpanel-wrap mb60">
        <el-form  class="tke-form m0" :model="pv" label-position='left' label-width="120px" size="mini"
         :rules="rules" ref="pv">
          <el-form-item label="来源设置" class="m0">
            <el-radio-group v-model="pv.tabPosition" style="margin-bottom: 30px;" @change="getNull()">
          ...
          <div v-if="pv.tabPosition=='jt'">
            ...
            <div v-if="pv.ps=='CBS'">
              ...
              <el-form-item label="选择云盘" class="m0">
                <div v-show="radioName ==''?true:false">未选择数据盘<a @click="dialogFormVisible=true" style="cursor:pointer;">选择云硬盘</a></div>
                <div  v-show="radioName !=''?true:false">{{radioName}}<a @click="dialogFormVisible=true" style="cursor:pointer;">重新选择</a></div>
              </el-form-item>
        ...
        <div class="tke-formpanel-footer">
          <el-button size="small" type="primary" @click="submitForm('pv')" :disabled="pv.tabPosition == 'dt'">创建PersistentVolume</el-button>
...
import { ALL_CITY,TKE_DESCRIBEDISKS,TKE_DESCRIBEZONES,POINT_REQUEST } from "@/constants";
export default {
  name: "pvCreate",
  data() {
  ...
  methods: {
    ...
    getAllData(){
        ...
        this.radioName = "["+allData.DiskSize+"GB"+"]"+allData.DiskId+"("+allData.DiskName+")"
       ...
       setValue() {
        var params = {
           ClusterName: this.$route.query.clusterId,
            ...
            RequestBody: JSON.stringify({"kind":"PersistentVolume","apiVersion":"v1","metadata":{"name":this.pv.name},"spec":{"accessModes":["ReadWriteOnce"],"storageClassName":this.pv.storageValue,"capacity":{"storage":this.DiskSize},"qcloudCbs":{"cbsDiskId":this.DiskId,"fsType":this.pv.radio}}}),
            Version: "2018-05-25"
        }
        this.axios.post(POINT_REQUEST, params).then(res => {
          if (res.Response.Error === undefined) {
            ...
            if(res.Response.Error.Code == "InternalError" && JSON.parse(res.Response.Error.Message).reason == "AlreadyExists"){
              this.see = true
            }
            this.$message({
              message: ErrOr[res.Response.Error.Code],
