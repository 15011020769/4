<template>
  <div>
          ...
          <el-col :span="12">
            <div class="informationList">
              <p>
                <span>域名包</span>
                <span
                  >{{ (package.DomainPkg && package.DomainPkg.Count) || 0
                  }}{{ t("个", "WAF.g") }} ({{
                    t(
                      "每个域名包包含10个域名防护，仅支持1个一级域名",
                      "WAF.mgymb"
                    )
                  }})
                  <a href="#" class="blueHref" @click="buyDominPackBtn">
                    <template v-if="package.DomainPkg">{{
                      t("购买域名包", "WAF.gmymb")
                    }}</template>
              ...
              <p>
                <span>已使用域名</span
                ><span
                  >{{ package.DomainCount }}/{{ package.DomainLimit
                  }}{{ t("个", "WAF.g") }}</span
                >
              </p>
              <p>
              ...
              <p>
                ...
                <span
                  ...
                  <a class="orangeHref">
                    {{
                      package.Level &&
                        CLB_PACKAGE_CFG_TYPES[package.Level].busQps +
                          ((package.QPS && package.QPS.Count) || 0)
                    }} </a
                  >，
                  <a href="#" class="blueHref" @click="qpsBack">
                    <template v-if="package.QPS">{{
                      t("立即升级", "WAF.ljsj")
                    }}</template>
                    <template v-else>{{ t("立即购买", "WAF.ljgm") }}</template>
                  </a>
    ...
    <buyDominPack
      :package="package"
      :isShow="buyDominModelShow"
      @buyDominPacModelClose="buyDominPacModelClose"
    />
    <RenewModel
      :package="package"
      :isShow="RenewModelShow"
      @renewModelClose="renewModelClose"
    />
    <buyLogBackModel
      :package="package"
      :isShow="buyLogBackModel"
      @closeLogBackModel="closeLogBackModel"
    />
    <qpsBackModel
      :package="package"
      :isShow="qpsBackModel"
      @closeqpsModel="closeqpsModel"
    />
  </div>
</template>
<script>
