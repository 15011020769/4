    ...
    <div class="wrapper">
      ...
      >
        <el-input
          v-model="iptDomain"
          style="width: 180px;"
          :placeholder="t('请输入域名', 'WAF.qsrym')"
        >
          <i
            slot="suffix"
            @click="getDescribeHost"
            ...
            <div slot="header">
              <el-dropdown trigger="click" style="line-height: 0">
                <span class="el-dropdown-link" style="font-size: 12px;">
                  BOT{{ t("流量分析开关", "WAF.llfxkg")
                  }}<i class="el-icon-arrow-down el-icon--right"></i>
                </span>
                <el-dropdown-menu @click="getDescribeHost" slot="dropdown">
                  <el-dropdown-item
                    ><span @click="onBotSearch(undefined)"
                      >全部</span
                    ></el-dropdown-item
                  >
                  <el-dropdown-item
                    ><span @click="onBotSearch('on')">{{
                      t("开启", "WAF.open")
                    }}</span></el-dropdown-item
                  >
                  <el-dropdown-item
                    ><span @click="onBotSearch('off')">{{
                      t("关闭", "WAF.close")
                    }}</span></el-dropdown-item
                  >
                </el-dropdown-menu>
          ...
          <el-table-column>
            <div slot="header">
              ...
              >
                ...
                <span slot="content">{{
                  t(
                    "WAF开关状态，可前往【Web应用防火墙-防护设置】域名列表页面进行设置",
                    "WAF.kqwwafyyfhq"
                  )
                }}</span>
              </el-tooltip>
            ...
            <div slot-scope="scope">
              <span
                :style="
                  `color: ${scope.row.Status > 0 ? '#13ce66' : '#e1504a'}`
                "
                >{{
                  scope.row.Status > 0
                    ? t("开启", "WAF.open")
          ...
          <el-table-column prop="action" label="操作">
            <template slot-scope="scope">
              <a
                ...
                @click="$router.push(`${$route.path}/diy/${scope.row.Domain}`)"
                >{{ t("防护设置", "WAF.fhsz") }}</a
              >
            </template>
          </el-table-column>
