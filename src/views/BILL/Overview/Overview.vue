<template>
  <div id="Overview">
    <div class="ovew">
      <span class="bill" style="font-size: 16px; font-weight: 700; line-height:3;">{{$t('BILL.Overview.title')}}</span>
      <el-date-picker v-model="month" type="month" value-format="yyyy-MM-dd" size="small" @change="getDataChar()"
    ...
    <el-card class="ovew-echar">
      <el-row :gutter="20">
        <el-col :span="18">
          ...
          <span
            style="font-size: 12px; color: #888;  margin-left: 5px; line-height:1">{{$t('BILL.Overview.unit')}}</span>
        ...
        <el-col :span="6">
          <el-form>
            <span style="font-size: 16px; color: #000; line-height: 50px;">{{this.month.split('-')[0]}} 年
              {{this.month.split('-')[1]}} 月</span>
            <el-form-item style="height: 20px;">
              <span style="font-size: 12px; color: #666;">{{$t('BILL.Overview.statCycle')}}</span>
              <span style="float: right; font-size: 12px; color: #666;">{{$t('BILL.Overview.feeCycle')}}</span>
            </el-form-item>
            <el-form-item style="height: 20px;">
              <span style="font-size: 12px; color: #666;">{{$t('BILL.Overview.totalAmount')}}</span>
              <span style="font-size: 16px; float: right; color: #006EFF;">{{this.total}}</span>
            </el-form-item>
            <span style="float: right;">=</span>
            <el-form-item style="height: 20px;">
              <span style="font-size: 12px; color: #666;">{{$t('BILL.Overview.cashAmount')}}</span>
              <span style="font-size: 16px; float: right;">{{this.cash}}</span>
            </el-form-item>
            <span style="float: right;">+</span>
            <el-form-item style="height: 20px;">
              <span style="font-size: 12px; color: #666;">{{$t('BILL.Overview.incentiveAmount')}}</span>
              <span style="font-size: 16px; float: right;">{{this.incentive}}</span>
            </el-form-item>
            <span style="float: right;">+</span>
            <el-form-item style="height: 20px;">
              <span style="font-size: 12px; color: #666;">{{$t('BILL.Overview.voucherAmount')}}</span>
    ...
    <el-card>
      <span
        style="font-size: 14px; font-weight: 700; line-height:1">{{this.month.split('-')[0]}}年{{this.month.split('-')[1]}}月{{$t('BILL.Overview.billSum')}}</span>
      ...
      <div>
        <el-tabs v-model="activeName" type="card">
          <el-tab-pane :label="$t('BILL.Overview.productSum')" name="first">
            ...
            <div class="paging">
              <el-pagination @size-change="sizeChangeHandle" @current-change="currentChangeHandle"
                :current-page="pageIndex" :page-sizes="[10, 20, 50, 100]" :page-size="pageSize" :total="totalPage1"
          ...
          <el-tab-pane :label="$t('BILL.Overview.projectSum')" name="second" style="width:100%">
            ...
            <el-table :data="dataList2" row-key="id" :tree-props="{children: 'children'}" v-loading="dataListLoading"
              style="width: 100%;">
              ...
              <el-table-column prop="state" align="center" :label="$t('BILL.Overview.costTrend')">
                <template slot-scope="scope">
                  <div slot="reference">
                    <el-popover placement="left" width="700" ref="popover" trigger="hover">
                      <div slot="reference">
                        <svg t="1574819723346" class="icon" viewBox="0 0 1024 1024" version="1.1"
                          xmlns="http://www.w3.org/2000/svg" p-id="2324" width="40" height="20">
                          <path
                            d="M664.1 783c-3.8 0-7.6-0.2-11.3-0.5-36.3-3.1-68.9-20.6-96.9-52.1-28.4-32-52.5-79.4-71.4-140.8-32.4-105-66.1-182.6-100.3-230.6-25.5-35.8-50.2-54-73.4-54h-0.4c-38.8 0.4-84.8 51-129.3 142.7-37 76.2-59.4 153-59.7 153.8L64 582.6c1-3.4 24.3-83.1 63.7-164.3 56.8-117 118.1-176.6 182.1-177.3h1c43 0 83.8 26.7 121.2 79.3 38.2 53.7 75.1 137.5 109.5 249.3 29.3 94.9 68.3 145.1 116 149.1 21.3 1.8 45.6-5.2 72.2-20.9 24.5-14.4 50.3-35.8 76.8-63.5 49.6-51.9 87.7-111.9 100.1-137.6L960 526c-14.6 30.4-56.4 96.4-111.4 154-30.4 31.8-60.6 56.6-89.9 73.9-32.8 19.3-64.6 29.1-94.6 29.1z"
                            fill="#1296db" p-id="2325"></path>
            ...
            <div class="paging">
              <el-pagination @size-change="sizeChangeHandle" @current-change="currentChangeHandle"
                :current-page="pageIndex" :page-sizes="[10, 20, 50, 100]" :page-size="pageSize" :total="totalPage2"
                layout="total, sizes, prev, pager, next, jumper" style="float: right;">
              </el-pagination>
            </div>
          </el-tab-pane>
    ...
    methods: {
      ...
      initChartBar(param) {
        ...
        this.$axios.post(`${process.env.VUE_APP_adminUrl}taifucloud/tbillproduct/costTrend?date=` + this.month +
          `&year=` + this.yearvalue + `&costType=` + this.value + `&uin=` + this.$cookie.get('uin')).then(data => {
          var list = data.chart
          const months = []
          var totalAmounts = []
      ...
      initLine1() {
        this.dataList1.forEach((row, index) => {
          ...
          this.$axios.post(`${process.env.VUE_APP_adminUrl}taifucloud/tbillproduct/getLineList?date=` + this.month +
            `&uin=` +
        ...
        this.dataList2.forEach((row, index) => {
          ...
          this.$axios.post(`${process.env.VUE_APP_adminUrl}taifucloud/tbillproduct/getLineList?date=` + this.month +
            ...
            row.children.forEach((value, index) => {
              // console.log(row.id)
              this.$axios.post(`${process.env.VUE_APP_adminUrl}taifucloud/tbillproduct/getLineList?date=` +
                this.month + `&uin=` +
                this.$cookie.get('uin') + `&projectName=` + project + `&businessCodeName=` + value
                .project_name).then(data => {
                var list = data.chart
                var xMonth = []
                var yTotalAmounts = []
                var yCastAmounts = []
