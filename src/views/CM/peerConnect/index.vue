          ...
          <template slot-scope="scope">
            <p>
              <a @click="jump(scope.row.peeringConnectionId)"
                style="cursor:pointer;">{{scope.row.peeringConnectionId}}</a>
            </p>
            <p>{{scope.row.peeringConnectionName}}</p>
          </template>
        </el-table-column>
...
import { DISK_CITY, PEE_LIST } from '@/constants';
export default {
  data() {
    return {
      cities: [],
  ...
  methods: {
    ...
    GetTabularData() {
      ...
      this.axios.post(PEE_LIST, param).then((data) => {
