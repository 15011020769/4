...
export default {
  ...
  methods: {
    GetDat(data) {
      ...
      this.value = data[2];
      const metricNArr = ["Outbandwidth", "Inbandwidth", "Outpkg", "Inpkg"];
      const symbol = ["Mbps", "Mbps", "个/秒", "个/秒"];
      this.tableData = [];
  ...
  filters: {
    ...
    UpTitle(value) {
      if (value === "Outbandwidth") {
        return (value = "VPN网关平均每秒出流量");
      }
      if (value === "Inbandwidth") {
        return (value = "VPN网关平均每秒入流量");
      }
      if (value === "Outpkg") {
        return (value = "VPN网关平均每秒出包量");
      }
      if (value === "Inpkg") {
        return (value = "VPN网关平均每秒入包量");
      }
      if (value === "") {
        return (value = "");
      }
