<template>
  <div class="defaultDetail">
    ...
    <product-type-cpt v-on:PassData="passData" :searchParam="searchParam" :projectId="projectId"
      :productValue="ViewName" style="display:none;" v-on:loading="Type_loading" />
    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix" style="width:100%;display:flex;">
        <h3>告警觸發條件</h3>
        <a v-loading="gaoJingLoading" v-if="gaoJingLoading1" class="gao-jing-loading">編輯</a>
        <a @click="editGaoJing" v-if="!gaoJingLoading1" class="gao-jing-loading">編輯</a>
      </div>
      <div class="box-content">
        ...
        <span class="textColor"
          v-if="basicNews.ConditionsConfig">指標告警（{{ basicNews.IsUnionRule === 0 ? "任意" : "所有" }}）</span>
        <div v-for="i in basicNews.ConditionsConfig" class="trigger-condition" :key="i.MetricShowName">
          <p>
            {{ i.MetricShowName }}
        ...
        <div v-for="(j, q) in basicNews.EventConfig" class="trigger-condition" :key="q + 1">
          <p>
            {{ j.EventShowName }}，{{
    ...
    <el-card class="box-card alarm-object" v-if="ViewName !== 'BS'">
      ...
      <div class="box-content" v-if="!InstanceGroupShow.InstanceGroup">
        <el-row style="margin:10px 5px;padding-top:10px">
          <el-button type="primary" @click="AlarmObjectNews">新增物件</el-button>
          <el-button v-if="UniqueId.length > 0" @click="AlarmObjectRemoval('', 2)">解除</el-button>
          <el-button disabled v-else>解除</el-button>
          <el-button @click="alarmObjectRemovalAllVisible = true">全部解除</el-button>
        </el-row>
      </div>
      <div class="alarm-object-table" v-if="!InstanceGroupShow.InstanceGroup">
        <el-table ref="multipleTable" :data="alarmObjectData" style="width: 100%" v-loading="alarmObjecLoad"
          ...
          <el-table-column label="IP地址" v-if="ViewName === 'cvm_device'">
            <template slot-scope="scope">
              <p>
                <span v-for="(item, index) in scope.row.PrivateIpAddresses" :key="index">
                  {{ item }}(內網)
                </span>
              </p>
              <p>
                <span v-for="(item, index) in scope.row.PublicIpAddresses" :key="index">
                  {{ item }}(外網)
                </span>
              </p>
            </template>
          ...
          <el-table-column prop="address" label="操作">
            <template slot-scope="scope">
              <a href="javascript:;" @click="AlarmObjectRemoval(scope.row, 1)">移出</a>
            </template>
          </el-table-column>
        </el-table>
        <!-- 分頁 -->
      ...
      <div class="alarm-object-table-instance" v-if="InstanceGroupShow.InstanceGroup">
        <ul>
          <li>
            <span>實例組</span>
            <span>{{ InstanceGroupShow.InstanceGroup.GroupName }}</span>
            <a href="javascript:;" @click="unBindingInstance = true">解除綁定</a>
          </li>
          <li>
            <span>實例數</span>
            <span>{{ InstanceGroupShow.InstanceGroup.InstanceSum }}個(啓用告警：
            ...
            <div>
              <div class="left">
                <div class="left-main border">
                  ...
                  <el-table :data="alarmObjectData" height="400" ref="multipleTable" class="table-left"
                      ...
                      <template slot-scope="scope">
                        <p>
                          <span v-for="(item, index) in scope.row
                              .PrivateIpAddresses" :key="index">{{ item }}(內網)</span>
                        </p>
                        <p>
                          <span v-for="(item, index) in scope.row.PublicIpAddresses" :key="index">{{ item }}(外網)</span>
                        </p>
                      </template>
                    </el-table-column>
                    <!-- BS -->
                    ...
                    <el-table-column label="內網地址" v-if="ViewName === 'dcchannel'">
                      <template slot-scope="scope">
                        {{ scope.row.WanIp }}
                      </template>
                    </el-table-column>
                    ...
                    <el-table-column label="啓用告警">
                      <template slot-scope="scope">
                        <el-switch v-model="scope.row.IsShielded" active-color="#006eff" inactive-color="#888"
    ...
    <el-card class="box-card alarm-object alarm-receiving-object">
      ...
      <div class="box-content" style="margin-top:20px;">
        <el-row>
          ...
          <el-button @click="Remove('', 2)" v-if="remove.length > 0">解除</el-button>
          <el-button disabled v-else>解除</el-button>
        </el-row>
      </div>
      <div class="alarm-object-table">
        <el-table ref="multipleTable" :data="receivingObjectData" style="width: 100%" height="300px"
          v-loading="receivingObjectLoad" @selection-change="receivingSelectionChange">
          <el-table-column type="selection"> </el-table-column>
          <el-table-column label="接收組" v-if="
          ...
          <el-table-column label="接收人" v-if="
              receivingObjectData.length === 0 ||
                ReceiverInfos.ReceiverType === 'group'
            ">
            <template slot-scope="scope">
          ...
          <el-table-column label="有效時段" v-if="
              receivingObjectData.length === 0 ||
                ReceiverInfos.ReceiverType === 'group'
            ">
          ...
          <el-table-column label="告警管道" v-if="
              receivingObjectData.length === 0 ||
                ReceiverInfos.ReceiverType === 'group'
            ">
          ...
          <el-table-column label="接收人" v-if="ReceiverInfos.ReceiverType === 'user'">
            <template slot-scope="scope">
              <div>
                {{ scope.row.Name }}
              </div>
          ...
          <el-table-column label="有效時段" v-if="ReceiverInfos.ReceiverType === 'user'">
            {{ ReceiverInfos.StartTime | EndTime }} -
            {{ ReceiverInfos.EndTime | EndTime }}
          </el-table-column>
          <el-table-column label="告警管道" v-if="ReceiverInfos.ReceiverType === 'user'">
            <span v-for="(item, index) in ReceiverInfos.NotifyWay" :key="index">
              {{ item | NotifyWay
              }}<i v-if="index != ReceiverInfos.NotifyWay.length - 1">、</i>
            </span>
    ...
    <el-card class="box-card">
      ...
      <div class="box-content" style="line-height: 30px;margin:20px 0;" v-if="!basicNews.Callback">
        未配置
      ...
      <div class="Callback-box-content" v-if="basicNews.Callback">
        ...
        <i class="el-icon-refresh ml5" v-if="Verification" @click="CallBackRefresh"></i>
        ...
        <div class="bg-box" style="width:40%;margin-top:16px;">
          <p>
            ...
            <el-tooltip effect="light" content="觸發驗證後推送請求三次，每次3s，累計9s未成功返回即驗證超時。" placement="top"><i
    ...
    <!-- 修改名稱 -->
    <el-dialog title="修改告警策略名稱" :visible.sync="modifyNameDialogVisible" width="500px" custom-class="tke-dialog">
      <div class="edit-dialog">
        <el-input size="small" placeholder="請輸告警策略名稱，20字以內" v-model="GroupName" @input="EditTips" maxlength="20">
        </el-input>
    ...
    <!-- 修改備注 -->
    <el-dialog title="修改告警策略備注" :visible.sync="modifyRemarksDialogVisible" width="600px" custom-class="tke-dialog">
      <div class="edit-dialog">
        <el-input type="textarea" :rows="2" placeholder="請輸入告警策略備注，100字以內" v-model="remarksVal" maxlength="100">
        </el-input>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="NameSure">確定</el-button>
    ...
    <!-- 告警觸發條件編輯 -->
    <el-dialog title="修改告警觸發條件" :visible.sync="dialogEditGaojing" width="1024px">
      <div>
        <div class="trigger-condition">
          <span>觸發條件</span>
          <div class="chufaContent">
            <div>
              <el-radio v-model="radioChufa" label="1" @change="chufaTemplate">觸發條件範本</el-radio>
              <a style="margin-left:15px;" @click="$router.push({ path: '/Template' })">新增触发条件範本</a>
              <div class="content" v-if="radioChufa == 1">
                <p>
                  <el-select v-model="nameVal" style="width:150px;" @change="TriggerChange"
                    :disabled="nameVal === '當前策略下沒有觸發條件範本'">
                    <el-option v-for="item in triggerCondition" :key="item.GroupID" :label="item.GroupName"
                      :value="item.GroupID" label-width="40px"></el-option>
                  </el-select>
                ...
                <div v-if="nameVal !== '當前策略下沒有觸發條件範本'">
                  ...
                  <p>
                    <span>滿足</span>
                    <el-select disabled v-model="Conditions.IsUnionRule" style="width:90px;margin:0 5px;">
                      <el-option v-for="x in satisfy" :key="x.value" :label="x.name" :value="x.value"
                  ...
                  <!-- 在這裏進行便利，添加 -->
                  <ul class="ul-one">
                    <li v-for="(item, index) in Conditions.Conditions" :key="index">
                      <div>
                        if&nbsp;
                        <el-select v-model="item.MetricDisplayName" style="width:130px;" disabled>
                          <el-option v-for="i in typeOpt" :key="i.MetricId" :label="i.MetricShowName"
                            :value="i.MetricId" label-width="40px"></el-option>
                        </el-select>&nbsp;
                        <el-select v-model="item.Period" style="width:140px;" disabled>
                          <el-option v-for="(i, x) in cycle" :key="x" :label="i.label" :value="i.value"
                            label-width="40px"></el-option>
                        </el-select>&nbsp;
                        <el-select v-model="item.CalcType" style="width:70px;" disabled>
                          <el-option v-for="i in CalcType" :key="i.value" :label="i.label" :value="i.value"
                            label-width="40px"></el-option>
                        </el-select>&nbsp;
                        <div class="input-style-dis-box">
                          <el-input class="input-style-dis" :value="item.CalcValue" disabled />
                          <span class="input-p">{{ item.Unit }}</span>
                        </div>
                        &nbsp;
                        <el-select v-model="item.ContinueTime" style="width:130px;" disabled>
                          <el-option v-for="i in ContinueTime" :key="i.value" :label="i.label" :value="i.value"
                            label-width="40px"></el-option>
                        </el-select>&nbsp;
                        <div v-if="!basicNews.ConditionsTemp" style="display:flex;aline-items:center;">
                          <span style="width:40px;margin:0px;">then</span>&nbsp;
                          <el-select v-model="item.AlarmNotifyPeriod" style="width:180px;" disabled>
                            <el-option v-for="(x, i) in AlarmNotifyPeriod" :key="i" :label="x.label" :value="x.value"
                              label-width="40px"></el-option>
                          </el-select>
                        </div>
                        <el-popover placement="top" width="360" trigger="hover">
                          ...
                          <i class="el-icon-info" style="color:#888; margin:0 5px;" slot="reference"></i>
                        </el-popover>
                      </div>
                    </li>
                  </ul>
                  <div style="margin-top:10px;" v-if="basicNews.ConditionsTemp">
                    <span>then</span>&nbsp;
                    <el-select v-model="alarmNotifyPeriodVal" style="width:180px;" disabled>
                      <el-option v-for="(x, i) in AlarmNotifyPeriod" :key="i" :label="x.label" :value="x.value"
                        label-width="40px"></el-option>
                    </el-select>
                  ...
                  <ul>
                    <li v-for="(x, i) in Conditions.EventConditions" :key="i">
                      <div style="margin-left:15px;">
                        <el-select v-model="x.EventDisplayName" style="width:180px;margin-top: 6px;" disabled>
            ...
            <div>
              <el-radio v-model="radioChufa" label="2" @change="config">配置觸發條件</el-radio>
              <div class="content content-pz" v-if="radioChufa == 2">
                <div>
                  <p style="line-height:30px;">
                    <el-checkbox v-model="formWrite.checkedZhibiao" style="height:30px;">指標告警</el-checkbox>
                  </p>
                  <p>
                    <span>滿足</span>
                    <el-select v-model="formWrite.satisfyVal" style="width:90px;margin:0 5px;"
                  ...
                  <ul class="ul-one ul-two">
                    <li v-for="(item, index) in formWrite.arr" :key="index">
                      <div>
                        <!-- 类型 -->
                        if&nbsp;
                        <el-select v-model="item.typeVal" style="width:150px;" @change="TypeChange(index)"
                          :disabled="!formWrite.checkedZhibiao">
                          <el-option v-for="x in typeOpt" :key="x.MetricId" :label="x.MetricShowName"
                            :value="x.MetricId" label-width="40px"></el-option>
                        </el-select>&nbsp;
                        <!-- 周期 -->
                        <el-select v-model="item.censusVal" style="width:140px;" :disabled="!formWrite.checkedZhibiao"
                          @change="ChangeD(index)">
                          <el-option v-for="(x, i) in item.cycle" :key="i" :label="x.label" :value="x.value"
                            label-width="40px"></el-option>
                        </el-select>&nbsp;
                        <el-select v-model="item.calcTypeVal" style="width:60px;" :disabled="!formWrite.checkedZhibiao"
                          @change="ChangeD(index)">
                          <el-option v-for="x in CalcType" :key="x.value" :label="x.label" :value="x.value"
                            label-width="40px"></el-option>
                        </el-select>&nbsp;
                        <div class="input-style-dis-box">
                          <el-input-number class="input-style-dis" v-model="item.number" controls-position="right"
                            :min="0" :max="item.max" :disabled="!formWrite.checkedZhibiao" @blur="ChangeD(index)">
                          </el-input-number>

                          <el-tooltip :manual="true" placement="right" effect="light" :value="item.tooltip">
                        ...
                        &nbsp;
                        <el-select v-model="item.continuousCycleVal" style="width:110px;"
                          :disabled="!formWrite.checkedZhibiao">
                          <el-option v-for="x in continuousCycleOpt" :key="x.value" :label="x.label" :value="x.value"
                            label-width="40px"></el-option>
                        </el-select>&nbsp;
                        <div v-if="formWrite.satisfyVal === 0">
                          <span>then</span>&nbsp;
                          <el-select v-model="item.warningVal" style="width:150px;"
                            :disabled="!formWrite.checkedZhibiao">
                            <el-option v-for="item in warningOpt" :key="item.value" :label="item.label"
                              :value="item.value" label-width="40px"></el-option>
                          </el-select>
                          <el-popover placement="top" width="360" trigger="hover">
                            <div>
                              <p>
                      ...
                      <i v-if="formWrite.arr.length !== 1" class="el-icon-error" style="color:#888; margin:10px 5px;"
                        @click="delZhibiao(index)"></i>
                    </li>
                    <a @click="addZhibiao()" v-if="!addDis">添加</a>
                    <a v-if="addDis" style="color:#888;">添加</a>
                    <div v-if="formWrite.satisfyVal === 1">
                      <span>then</span>&nbsp;
                      <el-select v-model="formWrite.warningVal" style="width:150px;"
                        :disabled="!formWrite.checkedZhibiao">
                        <el-option v-for="item in warningOpt" :key="item.value" :label="item.label" :value="item.value"
                          label-width="40px"></el-option>
                      </el-select>
                      <el-popover placement="top" width="360" trigger="hover">
                        <div>
                ...
                <div v-if="eventOpt.length !== 0">
                  ...
                  <ul class="ul-one">
                    <li v-for="(i, x) in formWrite.gaoArr" :key="x">
                      <p>
                        <el-select v-model="i.eventVal" style="width:180px;margin:0 5px;"
                          :disabled="!formWrite.checkedGaojing">
                          <el-option v-for="item in eventOpt" :key="item.EventId" :label="item.EventShowName"
                            :value="item.EventId" label-width="40px"></el-option>
                        </el-select>
                      </p>
                      <i v-if="formWrite.gaoArr.length > 1" class="el-icon-error" style="color:#888; margin:0 5px;"
                        @click="delShijian(x)"></i>
                    </li>
                    <a @click="addShijian" v-if="formWrite.gaoArr.length !== eventOpt.length">添加</a>
                    <a v-if="formWrite.gaoArr.length === eventOpt.length" style="color:#888;">添加</a>
                  </ul>
                </div>
              </div>
            </div>
    ...
    <!-- 告警對象編輯 -->
    <el-dialog title="編輯告警物件" :visible.sync="dialogEditObject" width="1024px">
      <div class="edit-alarm-object">
        <el-radio-group v-model="editAlarmObjectRadio">
          <p>
            <el-radio label="1">全部物件</el-radio>
          </p>
          <p>
            <el-radio label="2">選擇部分物件
              <!-- (已選{{ multipleSelection.length }}個) -->
            </el-radio>
          </p>
          <p>
            <el-radio label="3">選擇實例組</el-radio>
          </p>
        </el-radio-group>
        <div class="table" v-if="editAlarmObjectRadio == 2">
          <CamTransferCpt :productData="productListData" v-on:projectId="projectIds" v-on:searchParam="searchParams"
            v-on:multipleSelection="selectDatas" :isShowRight="isShowRight" v-loading="loading"
            v-on:CAM_loading="CAM_loading"></CamTransferCpt>
        </div>
        <div class="table" v-if="editAlarmObjectRadio == 3">
          <el-select v-model="InstanceGroup" style="width:100px;">
            <el-option v-for="item in InstanceGroupOpt" :key="item.InstanceGroupId" :label="item.GroupName"
    ...
    <!-- 告警對象-新增對象 -->
    <el-dialog title="關聯告警物件" :visible.sync="alarmObjectNews" width="1024px">
      <div class="edit-alarm-object-news">
        <p>選擇部分物件</p>
        <div class="table">
          <CamTransferCpt :selectedList="alarmObjectData" :showSelectedList="true" :productData="productListData"
            v-on:projectId="projectIds" v-on:searchParam="searchParams" v-on:multipleSelection="selectDatas"
            :isShowRight="isShowRight" v-loading="loading" v-on:CAM_loading="CAM_loading"></CamTransferCpt>
        </div>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="EditObjectSave()">保 存</el-button>
    ...
    <!-- 告警對象移出 -->
    <el-dialog :title="alarmObjecTitle" :visible.sync="alarmObjectRemovalVisible" width="500px"
      custom-class="tke-dialog">
      <div class="edit-dialog">
        解除後，將不會收到告警
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="AlarmObjectRemovalSure">解除</el-button>
        <el-button @click="alarmObjectRemovalVisible = false">取消</el-button>
      </div>
    </el-dialog>
    <!-- 告警對象全部移出 -->
    <el-dialog title="確定解除當前地域下所有實例？" :visible.sync="alarmObjectRemovalAllVisible" width="500px"
      custom-class="tke-dialog">
      <div class="edit-dialog">
        解除後，將不會收到告警
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="AlarmObjectRemovalAllSure">全部解除</el-button>
        <el-button @click="alarmObjectRemovalAllVisible = false">取消</el-button>
      </div>
    </el-dialog>
    <!-- 告警接收對象 編輯 -->
    <el-dialog title="告警接收物件" :visible.sync="editReceiveObjectVisuble" width="800px" custom-class="tke-dialog">
    ...
    <!-- 回調接口 -->
    <el-dialog title="回調接口" :visible.sync="callbackInterface" width="550px" class="callback-dialog-box"
    ...
    <!-- 告警對象 解除綁定 -->
    <el-dialog title="確定釋放告警策略" :visible.sync="unBindingInstance" width="600px" custom-class="tke-dialog">
      <div>
        釋放後，策略將不再對實例分組生效
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="UnBindingInstance">確定釋放</el-button>
        <el-button @click="unBindingInstance = false">取消</el-button>
      </div>
    </el-dialog>
    <!-- 告警接收對象 解除 -->
    <el-dialog :title="relieveTitle" :visible.sync="relieveDialogVisible" width="600px" custom-class="tke-dialog">
      <div>
        確認解除告警{{
          ...
          if (this.basicNews.ConditionsConfig) {
            ...
            for (let i in _ConditionsConfig) {
              ...
              this.formWrite.arr.push({
                ...
                tooltip: false,
                continuousCycleVal: Number(_ConditionsConfig[i].ContinueTime) /
                ...
                for (let j in this.typeOpt) {
                  if (
                    ...
                    for (let k in this.formWrite.arr) {
                      ...
                      for (
                        let b = 0; b < this.formWrite.arr[k].cycle.length - 1; b++
                      ) {
                        for (
                          let c = b + 1; c < this.formWrite.arr[k].cycle.length; c++
                        ) {
                          if (
              ...
              if (this.ViewName === "cvm_device") {
                ...
                this.axios.post(REDIS_LIST, params).then(res => {
                  if (res.Response.Error === undefined) {
                    ...
                    let ErrTips = {
                      "InternalError.DbOperationFailed": "統壹的 DB 操作錯誤，可以是 update insert select..。",
                      InvalidParameter: "參數錯誤",
        ...
        this.axios.post(CM_ALARM_RECEIVE_OBJECT_RELIEVE, param).then(res => {
          if (res.Response.Error === undefined) {
            ...
            let ErrTips = {
              ...
              UnknownParameter: "未知參數錯誤。",
              UnsupportedOperation: "操作不支持。",
              UnsupportedProtocol: "HTTPS 請求方法錯誤，只支持 GET 和 POST 請求。",
