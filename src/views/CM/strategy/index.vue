<template>
  <div class="strategy-wrap">
          ...
          <div>
            ...
            <el-form-item>
              <div style="display: flex;">
                <el-select filterable v-model="formInline.product_name" class="select-option" disabled>
                  <el-option v-for="item in formInline.product_kind" :key="item.name" :label="item.name"
                    :value="item.id" label-width="40px"></el-option>
                </el-select>
                <el-select filterable v-model="formInline.productArr" class="select-option-tow"
                  v-if="formInline.product_name === '1'" multiple>
                  <el-option v-for="item in productOptions" :key="item.label" :label="item.label" :value="item.viewName"
                    label-width="40px"></el-option>
                </el-select>
                <el-select filterable v-model="productValue" class="select-option-tow"
                  v-if="formInline.product_name === '2'" @change="changeMonitor">
                  <el-option v-for="item in productOptions" :key="item.label" :label="item.label" :value="item.viewName"
                    label-width="40px"></el-option>
                </el-select>
              </div>
    ...
    <div class="table">
      <p class="addBtn">
        <el-row>
          ...
          <el-button v-if="multipleSelection.length > 0" @click="AllDelete">刪除</el-button>
          <el-button v-else disabled>刪除</el-button>
          <el-button v-if="ModifyAlarm" @click="ModifyAlarmBtn()">修改告警管道</el-button>
      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loadShow" height="450"
          ...
          <template slot-scope="scope">
            <el-popover placement="right" width="500" trigger="hover">
              <div class="popover-box">
                ...
                <div v-for="(i, item) in scope.row.Conditions" :key="item" class="trigger-condition">
                  <p>
                    {{ i.MetricShowName }}
                ...
                <div v-for="j in scope.row.EventConditions" class="trigger-condition">
                  <p>
                    {{ j.EventShowName }}，{{
              ...
              <div slot="reference">
                <div v-for="item in scope.row.Conditions" class="trigger-condition">
                  <p>
                    {{ item.MetricShowName }}
                ...
                <div v-for="(items, indexs) in scope.row.EventConditions" :key="indexs" class="trigger-condition">
                  <p>
                    {{ items.EventShowName }}，{{
        ...
        <el-table-column label="策略類型">
          <template slot-scope="scope">
            <el-popover placement="left-start" trigger="hover">
              <div>
                <p>
                  <span>策略類型： <span v-if="scope.row.IsDefault == 1">預設</span>{{ scope.row.Name }}</span>
                </p>
                <p v-if="
              ...
              <div slot="reference">
                <div :class="{ strong: scope.row.IsDefault == 1 }" @mouseenter="enter(scope.$index)"
                  @mouseleave="leave()">
                  <span v-if="scope.row.IsDefault == 1">預設</span>{{ scope.row.Name }}
                  <p v-show="edit && scope.$index == current"
                    v-if="scope.row.IsDefault != 1 && scope.row.CanSetDefault">
                    <a href="javascript:;" @click="SetDefault(scope.row)">設置預設</a>
                  </p>
                </div>
              </div>
            </el-popover>
        ...
        <el-table-column label="已啓用/實例數">
          <template slot-scope="scope">
            ...
            <div class="group-color" v-if="scope.row.InstanceGroup != undefined">
            ...
            <div v-if="scope.row.ReceiverInfos != undefined">
              <div v-for="(i, x) in scope.row.ReceiverInfos" :key="x">
                ...
                <p v-if="i.NotifyWay.length > 0">
                  管道：<span v-for="(j, k) in i.NotifyWay" :key="k">{{ j | NotifyWay
        ...
        <el-table-column label="告警啓停">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.IsOpen" active-color="#006eff" inactive-color="#888"
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="text" class="cloneBtn" @click="Copy(scope.row)">複製</el-button>
            <el-tooltip content="預設策略不支持刪除，可釋放所有資源或設置新的預設策略後將此轉爲非預設策略" placement="left" effect="light"
              v-if="scope.row.IsDefault != 0">
              <el-button type="text" class="deleteBtn-color deleteBtn">刪除</el-button>
            </el-tooltip>
            <el-tooltip content="釋放所有資源後支持刪除" placement="left" effect="light" v-else-if="scope.row.UseSum != 0">
              <el-button type="text" class="deleteBtn-color deleteBtn">刪除</el-button>
            </el-tooltip>
            <el-button type="text" class="deleteBtn" v-else @click="Delete(scope.row)">刪除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div style="display:none;">
        <el-table :data="tableData" style="width: 100%" height="450" id="exportTable">
          <el-table-column label="策略名稱">
            <template slot-scope="scope">
            ...
            <template slot-scope="scope">
              <el-popover placement="left-start" trigger="hover">
                <div>
                  <p>
                    <span>策略類型：
                      <span v-if="scope.row.IsDefault == 1">預設</span>{{ scope.row.Name }}</span>
                  </p>
                  <p v-if="
                      scope.row.IsDefault == 0 &&
                        scope.row.CanSetDefault === false
      ...
      <div class="tke-page">
        <div class="block">
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
            :current-page="pageIndex1" :page-sizes="[10, 20, 50, 100]" :page-size="pageSize"
    ...
    <!-- 修改名稱 -->
    <el-dialog title="修改告警策略名稱" :visible.sync="modifyNameDialogVisible" width="500px" custom-class="dialog-box">
    ...
    <!-- 刪除 -->
    <el-dialog title="策略刪除確認" :visible.sync="deleteDialogVisible" width="500px" custom-class="tke-dialog"
      class="dialog-box">
    ...
    <!-- 刪除策略 -->
    <el-dialog title="刪除策略" :visible.sync="deleteAllDialogVisible" width="600px" custom-class="tke-dialog"
      class="dialog-box">
      <div>
        <p>您已選擇{{ multipleSelection.length }}條策略，詳情如下:</p>
        <div class="delete-table">
          <el-table :data="multipleSelection" style="width: 100%" height="200px">
            ...
            <el-table-column label="備注">
              <template slot-scope="scope">
                <p class="text-color-red" v-if="scope.row.IsDefault != 0" id="text-color-red">
                  <span>預設策略,無法刪除</span>
                  <el-tooltip content="預設策略不支持刪除，可釋放所有資源或設置新的預設策略後將此轉爲非預設策略" placement="left" effect="light">
                ...
                <p class="text-color-red" v-else-if="scope.row.UseSum != 0" id="text-color-red">
                  <span>已關聯物件,無法刪除</span>
                  <el-tooltip content="釋放所有資源後支持刪除" placement="left" effect="light">
                    <i class="el-icon-info ml5"></i>
                  </el-tooltip>
                </p>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" v-if="!showIsDefault" @click="DeleteSure">確定</el-button>
        <el-button disabled v-if="showIsDefault">確定</el-button>
        <el-button @click="deleteAllDialogVisible = false">取消</el-button>
      </span>
    </el-dialog>
    <!-- 修改告警管道 -->
    <el-dialog title="修改告警管道" :visible.sync="ModifyDialogVisible" width="600px" custom-class="tke-dialog"
      class="dialog-box">
      <div>
        <p>
          <el-tooltip effect="light" content="告警管道爲空的策略不支持修改" placement="top">
        ...
        <div class="modify-box">
          <div>
            ...
            <el-select v-model="emailVal" placeholder="請選擇">
              <el-option v-for="item in emailOpt" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </div>
          <!-- <div>
    ...
    <!-- 告警啓停 -->
    <el-dialog title="告警操作確認" :visible.sync="startStop" width="500px" custom-class="tke-dialog"
