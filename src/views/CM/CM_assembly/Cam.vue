  ...
  <div class="qudaoContent">
    <p class="receiving-objects">
      ...
      <el-row class="seek" style="display:flex;margin:0 10px;">
        <el-input v-model="triggerInput" v-if="formInline.jieshou === '0'" placeholder="請輸入用戶組名稱"></el-input>
        <el-input v-model="triggerInput" v-if="formInline.jieshou === '1'" placeholder="請輸入接收人名稱"></el-input>
        <el-button icon="el-icon-search" style="margin-left:-1px;margin-top:1px;" @click="searchKey"></el-button>
      </el-row>
      <a href="../CAM/index.html#/UserGroup" v-if="formInline.jieshou === '0'">新增接收组</a>
      <a href="../CAM/index.html#/UserListNew" v-if="formInline.jieshou === '1'">新增接收人</a>
    </p>
    <div>
      <!-- 接收组table -->
      <el-table v-if="formInline.jieshou === '0'" :data="tableData2" v-loading="loadingShow" @selection-change="handleSelectionChange" style="width: 100%" height="430" :default-sort="{ prop: 'changeData', order: 'descending' }">
        ...
        <el-table-column label="用户名">
          <template slot-scope="scope">
            <span v-if="scope.row.UserInfo.length !== 0" v-for="(item,index) in scope.row.UserInfo">
              <span>{{item.Name}}</span>
