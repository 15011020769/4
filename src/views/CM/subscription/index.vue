  ...
  <div class="subscription-wrap">
    ...
    <div class="table">
      ...
      <el-table :data="tableData" style="width: 100%;" v-loading="loadShow">
        <el-table-column label="事件類型" width="240">
          <template slot-scope="scope">
            <div v-if="scope.row.AccidentName=='雲伺服器儲存問題'">
              ...
              <el-tooltip class="item" effect="light" content="雲伺服器基礎設施儲存模組; 影響：導致IO效能下降 ，數據讀寫異常"
            ...
            <div v-if="scope.row.AccidentName=='雲伺服器網路連接問題'">
              ...
              <el-tooltip class="item" effect="light" content="雲伺服器基礎設施網路模組導致; 影響：網速下降或網路連接中斷" placement="bottom-start">
                <i class="el-icon-info" style="margin:0 5px;cursor: pointer;"></i>
              </el-tooltip>
            </div>
            ...
            <div v-if="scope.row.AccidentName=='雲伺服器運列異常'">
              ...
              <el-tooltip class="item" effect="light" content="雲伺服器基礎設施導致; 影響：雲伺服器高負載或宕機，服務不可用"
                placement="bottom-start">
                <i class="el-icon-info" style="margin:0 5px;cursor: pointer;"></i>
              </el-tooltip>
            </div>
        ...
        <el-table-column label="郵件" width="180">
          <template slot-scope="scope">
            <i v-if="scope.row.NotifyWay" v-show="scope.row.NotifyWay[0] ? true :false" class="el-icon-circle-check"
              style="color:#0abf5b"></i>
            <span v-if="!scope.row.NotifyWay">尚未訂閱</span>
          </template>
        </el-table-column>
        <el-table-column label="站内信" width="180">
          <template slot-scope="scope">
            <i v-if="scope.row.NotifyWay" v-show="scope.row.NotifyWay[1] ? true :false" class="el-icon-circle-check"
              style="color:#0abf5b"></i>
          </template>
        </el-table-column>
        ...
        <el-table-column label>
          <template slot-scope="scope">
            ...
            <el-button type="text" class="btn unSubBtn" @click="cancel(scope.row)" v-if="scope.row.NotifyWay">取消訂閱
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="訂閱管理" :visible.sync="dialogSubscribe" width="640px">
      <div class="list">
        ...
        <el-table :data="userListArr" tooltip-effect="dark" v-loading="loadingShow1"
          @selection-change="handleSelectionChange2" style="width: 100%;" height="430"
