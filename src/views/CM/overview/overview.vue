        ...
        <div class="left">
          ...
          <div class="box">
            <div class="head">
              ...
              <el-select v-model="value1" :placeholder="$t('CVM.Dashboard.qxz')" style="width:140px;margin-left:-1px;"
                @change="getHealthStatusList">
                <el-option v-for="item in options1" :key="item.value" :label="item.label" :value="item.projectId">
                </el-option>
              </el-select>
            </div>
            <el-table v-loading="tableLoading" :data="productOptions" style="width: 100%" height="450"
              :empty-text="$t('CVM.clBload.zwsj')">
              <el-table-column prop="label" :label="$t('CVM.overview.fwlx')"></el-table-column>
              <el-table-column prop="status">
                <template slot="header">
                  <label>{{ $t("CVM.overview.dqzt") }}</label>
                  <el-tooltip placement="right" effect="light">
                    <div slot="content" style="line-height: 20px">
                      <i class="el-icon-success" style="color: green;font-size: 10px;"></i>
                      正常 無資源異常<br />
                      <i class="el-icon-question" style="color: red;font-size: 14px;"></i>
                      提醒 近24小時內有資源發生過異常<br />
                      <i class="el-icon-warning" style="color: red;font-size: 10px;"></i>
                      異常 當前有資源正處於異常狀態未修複
                    </div>
                    <i class="el-icon-info" style="font-size:15px;vertical-align: middle;"></i>
                  </el-tooltip>
                </template>
                <template slot-scope="scope">
                  <el-tooltip placement="right" effect="light" :disabled="scope.row.tips.length === 0"
                    popper-class="cm-overview-tooltip">
                    <timelineview slot="content" :data="scope.row.tips" :viewName="scope.row.viewName"
                      @itemClick="itemClick"></timelineview>
                    <i v-if="scope.row.status" class="el-icon-success"
                      style="color: green;font-size: 15px;margin-right:3px;"></i>
                    <i v-else class="el-icon-warning" style="color: red;font-size: 15px;margin-right:3px;"></i>
                  </el-tooltip>
              ...
              <el-table-column prop="address" :label="$t('CVM.overview.yxdxs')">
                <template slot-scope="scope">
                  <span v-if="scope.row.desc !== 0">{{ scope.row.subtitle + "："
                    }}<a @click="goToProduct(scope.row)" class="go-to-product">{{ scope.row.desc }}</a></span>
                  <span v-else>-</span>
          ...
          <div class="box">
            <div class="head">
              ...
              <el-select v-model="value2" :placeholder="$t('CVM.Dashboard.qxz')" style="width:140px;margin-left:-1px;"
                ...
                <el-option v-for="item in options2" :key="item.value" :label="item.label" :value="item.projectId">
                </el-option>
              </el-select>
              <type @PassData="PassData" :projectId="projectId" :searchParam="searchParam" :productValue="productValue"
                @loading="loading" />
              ...
              <div class="trail" v-if="chartsObjects.length > 0">
                <a href="#" @click="exportCSV">導出CSV</a><i class="el-icon-download"></i>
              </div>
            </div>
            <TimelineCharts :timelineData="timelineData" :loading="chartLoading" :day="monitorStartTime">
            </TimelineCharts>
            <div class="button-group">
              <el-button type="primary" v-for="day in sevenDays" :key="day.key" :plain="day.isSelected"
                @click="handleDaysButtonEvent($event, day)">{{ day.value.format("MM-DD") }}</el-button>
            </div>
          </div>
        </div>
    ...
    methods: {
      ...
      getHealthStatusList() {
        ...
        if (typeof this.value1 === "number") {
          params["ProjectIds.0"] = this.value1;
        }
      ...
      MonitorList() {
        ...
        if (typeof this.value2 === "number") {
          params["ProjectIds.0"] = this.value2;
        }
