  ...
  export default {
    data() {
      return {
        ...
        Tips: {
          'AccOuttraffic': '外網網路介面的平均每秒出流量',
          'BaseCpuUsage': ' 基礎CPU使用率通過宿主機採集上報，無須安裝監控元件即可檢視數據，子機高負載情況下仍可持續採集上報數據',
          'CpuLoadavg': '1分钟内CPU平均负载，取 /proc/loadavg 第一列數據（windows作業系統無此指標），依賴監控元件安裝採集',
          'CpuUsage': 'CPU利用率是通過CVM子機內部監控元件採集上報，數據更加精準',
          'DiskIoAwait': ' 磁碟分區I/O平均每次操作的等待時間',
          'DiskSvctm': '磁碟分區平均每次I/O操作所花的時間',
          'DiskUtil': '磁碟分區有IO操作的時間與總時間的百分比',
          'DiskWriteTrafficNew': '平均每秒從記憶體寫到磁碟的數據量',
          'LanInpkg': '內網網路介面的平均每秒入包量',
          'LanIntraffic': '內網網路介面的平均每秒入流量',
          'LanOutpkg': '內網網路介面的平均每秒出包量',
          'LanOuttraffic': '內網網路介面的平均每秒出流量',
          'MemUsage': '用戶實際使用的記憶體量與總記憶體量之比，不包括緩衝區與系統快取佔用的記憶體',
          'MemUsed': ' 使用的記憶體量，不包括系統快取和快取區佔用記憶體，依賴監控元件安裝採集',
          'TcpCurrEstab': '處於 ESTABLISHED 狀態的 TCP 連接數量，依賴監控元件安裝採集',
          'WanInpkg': '外網平均每秒入包量',
