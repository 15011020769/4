<template>
  <div class="detail-audit">
    <div class="header">
      ...
      {{title}}
      <el-switch v-model="value" :inactive-text="$t('CLA.total.logOn')" style="float:right;margin-top:5px;"
        @change="_switch"></el-switch>
    </div>
    <!-- 弹出框 -->
    ...
    <el-form :model="detailData" status-icon :rules="rules" ref="detailData" label-width="100px" class="demo-ruleForm"
      ...
      <div class="detil-audit-main">
        <div class="detil-audit-box" v-loading="boxloading1">
          <span class="edit" @click="_edit1" v-if="!inpShow1">{{ $t('CLA.total.bj') }}</span>
          ...
          <div class="inp-box" v-if="inpShow1">
            ...
            <el-form-item :label="$t('CLA.total.coscct')" required class="select">
              <el-select v-model="select.name" :placeholder="$t('CLA.total.qxz')" @change="_select">
                <el-option v-for="(item,index) in select.options" :key="index" :label="item.CosRegionName"
                  :value="item.CosRegion">
                </el-option>
              </el-select>
