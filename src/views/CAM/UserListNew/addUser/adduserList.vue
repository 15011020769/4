    ...
    <div class="adduserlist-main" v-loading="loading">
      ...
      <div class="main">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
          <div class="step2" v-show="active == 1">
            ...
            <el-form-item label="備註" prop="Remark">
              <el-input style="width: 330px" v-model="ruleForm.Remark"
                :placeholder="$t('CAM.userList.userRemarkPlaceholder')"></el-input>
              ...
              <el-form-item :label="$t('CAM.userList.ConsolePassword')" required>
                ...
                <div v-show="pwdInp" class="regPWD">
                  <el-input v-model="ruleForm.Password" style="width:200px;" type="password" show-password
                    maxlength="32" @click="visible = !visible" @change="_pwdInp"></el-input>
                  <div class="reg-box">
        ...
        <div class="step4" v-if="active == 3">
          <Step4 ref="step4" :userInfo.sync="this.ruleForm" :strategies="strategies" />
        </div>
      </div>
      <div class="btn-box">
    ...
    data() {
      ...
      return {
        ...
        type: [{
            code: 1,
            title: "可訪問資源並接收消息",
            txt: "該用戶可以登錄控制台或通過 API 金鑰訪問您授予其許可權的台富雲資源，同時擁有接收消息等子帳號的全部功能"
          },
          {
            code: 0,
            title: "僅用於接收消息",
            txt: "該用戶僅可通過手機、郵箱接收台富雲發送給您的消息通知，不可訪問台富雲"
          }
        ],
        //访问方式
        visitType: false,
    ...
    methods: {
      ...
      async addUser() {
        ...
        const param = {
          ...
          useApi: 1,
          uin: this.$cookie.get("uin"), //uin
        };
        let res = await this.axios.post(`${process.env.VUE_APP_adminUrl}taifucloud/account-sub/new/manage/register`,
          param)
        if (res.ErrCode) {
          this.$message({
            message: res.ErrMsg,
