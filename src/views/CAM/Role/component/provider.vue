  ...
  <el-form :model="form" ref="form" label-width="100px">
    ...
    >
      <el-select
        v-model="form.name"
        filterable
        :placeholder="$t('CAM.Role.zwkxsftgs')"
        size="small"
      >
        <el-option
  ...
  watch: {
    removeProviders: {
      handler() {
        ...
        this.axios.post(LIST_PROVIDERS, params).then(res => {
          const p = this.removeProviders.map(p => p.name);
          this.providers = res.Response.SAMLProviderSet.filter(
            smal => !p.includes(smal.Name)
          );
        });
      },
      immediate: true
