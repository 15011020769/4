<template>
  <div class="RoleDetail wrap">
    ...
    <div class="container">
        ...
        <div class="baseInfo_flex">
          ...
          <p>
            ...
            <span style="display:block;flex:1;margin-top:-3px;">
              <el-input v-if="input_show" v-model="roleInfo.Description" size="mini" style="width:150px"
                :placeholder="$t('CAM.strategy.inputContent')"></el-input>
              <a v-if="input_show" @click="input_sure" style="margin-left:10px"
                href="javascript:;">{{ $t("CAM.userGroup.delConfirmBtn") }}</a>
              <a v-if="input_show" @click="input_cancel" style="margin-left:10px" href="javascript:;">取消</a>
              <span v-if="!input_show" style="line-height:20px;">{{
      ...
      <div class="tabs" v-loading="loading">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          ...
          <el-tab-pane :label="$t('CAM.Role.Authorized')" name="first">
            <p style="margin:10px">
              <el-button type="primary" @click="relationPolicies" size="small">{{ $t("CAM.userList.RelatedPolicies") }}
              </el-button>
              <el-button type="primary" @click="relieveRolePolicies" size="small" :disabled="displayPolicies">批量解除策略
              </el-button>
            </p>
            <div class="first_table">
              <el-table @selection-change="handleSelectionChangePolicies" :data="rolePolicies" height="300"
                style="width: 100%" v-loading="loading">
                <el-table-column type="selection" width="29"></el-table-column>
                <el-table-column prop="PolicyName" label="策略名">
                  <template slot-scope="scope">
                    <el-button @click="first_handleClick(scope.row)" type="text" size="small">{{ scope.row.policyName }}
                    </el-button>
                  </template>
                </el-table-column>
                <el-table-column align="center" label="策略類型">
                  <template slot-scope="scope">
                    <div v-if="scope.row.policyType == 'User'">
                      {{ $t("CAM.userList.strategySelf") }}
                    </div>
                    <div v-else-if="scope.row.policyType == 'QCS'">
                      {{ $t("CAM.userList.ysStrategy") }}
                    </div>
                  </template>
                </el-table-column>
                ...
                <el-table-column>
                  ...
                  <template slot-scope="scope">
                    <span v-if="scope.row.validTime" style="color: #e54545">{{
                      scope.row.validTime
                    }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    <el-button @click.native.prevent="relieveRolePolicy(scope.row, true)" type="text" size="small">解除
                    </el-button>
                  </template>
                </el-table-column>
              </el-table>
              <div class="Right-style pagstyle" v-show="activeName == 'first'"
                ...
                <el-pagination :page-size="pagesize" :pager-count="7" layout="prev, sizes, pager, next"
                  :page-sizes="[10, 20, 30, 40, 50]" @current-change="handleCurrentChange"
                  @size-change="handleSizeChange" :total="TotalCount">
                </el-pagination>
              </div>
            </div>
          </el-tab-pane>
          ...
          <el-tab-pane :label="$t('CAM.Role.roleCarrier')" name="second">
            <div class="config">
              <p style="margin:10px">
                <el-button type="primary" @click="Relation_user" size="small">{{
                  $t("CAM.Role.Management")
                }}</el-button>
              </p>
              <div class="config_table">
                <el-table :data="
                  ...
                  " height="300" :row-style="{ height: 0 }" :cell-style="{ padding: '5px 10px' }"
                  :header-cell-style="{ height: '20px', padding: '0px 10px' }" style="width: 100%">
                  ...
                  <el-table-column label="條件" v-if="roleCarrierType === 'federated'">
                    <template slot-scope="scope" show-overflow-tooltip>
                      <span v-if="condition[scope.$index].length">
                        {{ condition[scope.$index][0].key }} -
                        {{ condition[scope.$index][0].condi }} -
                        {{ condition[scope.$index][0].val }}
                        <el-button type="text" :disabled="roleCarrierType !== 'federated'" @click="
                            handleRolePayloadRelationVisible(
                              scope.row,
                              condition[scope.$index]
                            )
                          " style="font-size: 12px;" v-if="condition[scope.$index].length > 1">以及({{
                            condition[scope.$index].length - 1
                          }})項</el-button>
                      </span>
                      <span v-else>-</span>
                    </template>
                  ...
                  <el-table-column label="操作" width="180">
                    <template slot-scope="scope">
                      <el-tooltip v-if="roleCarrier.length === 1" effect="dark" :content="$t('CAM.Role.wfscwydzt')"
                        placement="top-start">
                        <el-button type="text" size="small">解除</el-button>
                      </el-tooltip>
                      <el-button v-if="roleCarrier.length > 1" @click.native.prevent="
                          delRolePolicy(scope.$index, roleCarrier,scope.row)
                        " type="text" size="small">解除</el-button>
                      <el-button type="text" size="small" v-show="roleCarrierType === 'federated'" @click="
                          handleRolePayloadRelationVisible(
                            scope.row,
                            condition[scope.$index]
                          )
                ...
                <div class="Right-style pagstyle" v-show="activeName == 'second'"
                  style="height:70px;display:flex;align-items:center;">
                  <span class="pagtotal">共&nbsp;{{ TotalCounts }}&nbsp;{{ $t("CAM.strip") }}</span>
                  <el-pagination :page-size="pagesize" :pager-count="7" layout="prev, sizes, pager, next"
                    :page-sizes="[10, 20, 30, 40, 50]" @current-change="handleCurrentChange"
                    @size-change="handleSizeChange" :total="TotalCounts">
                  </el-pagination>
                </div>
              </div>
            </div>
            ...
            <div class="explain">
              <p>
                {{ $t("CAM.Role.cancelSessionTitle1") }}RevokeOlderSessionFor{{
                  roleInfo.RoleName
                }}{{ $t("CAM.Role.cancelSessionTitle2") }}
              </p>
            </div>
            <el-button
              disabled
      ...
      <el-dialog :visible.sync="dialogVisible" width="25%" :before-close="handleCloseSessionHint">
        ...
        <p style="text-align:center" slot="footer">
          ...
          <el-button type="primary" @click="
              Relievesure_dialogVisible = true;
              dialogVisible = false;
            " size="small">{{ $t("CAM.userList.suerAdd") }}</el-button>
        </p>
      </el-dialog>
      <!-- 载体dialog  start -->
      ...
      <el-dialog :visible.sync="Relieve_dialogVisible" width="30%" :before-close="handleCloseCarrier">
        ...
        <div style="margin:15px 0">
          <p>
            ...
            <a href="javascript:;" @click="look_detail">{{
              $t("CAM.strategy.lookDetils")
            }}</a>
            <i v-if="!isShow" class="el-icon-caret-bottom"></i>
            <i v-if="isShow" class="el-icon-caret-top"></i>
          </p>
          <div v-if="isShow" class="box">
            <p class="list" v-for="item in RelieveData" :key="item">
              {{ item }}
            </p>
          </div>
        ...
        <p style="text-align:center">
          ...
          <el-button type="primary" @click="Relieve_dialogVisible = false" size="small">
            {{ $t("CAM.userGroup.delConfirmBtn") }}</el-button>
        </p>
      </el-dialog>
      <el-dialog :visible.sync="dialogVisiblePolicies" width="1200px" :before-close="handleClosePolicy"
        destroy-on-close>
        <p class="dialog" slot="title">
          {{ $t("CAM.userGroup.createRelevance") }}
        </p>
        <transfer :roleId="roleId" ref="transfer"></transfer>
        <p style="text-align:center;margin-top:30px">
          <el-button @click="dialogVisiblePolicies = false" size="small">取 消</el-button>
          <el-button type="primary" @click="attachRolePolicies" size="small">{{
            $t("CAM.userGroup.delConfirmBtn")
          }}</el-button>
        </p>
      </el-dialog>
      <!-- 撤销所有会话弹窗 -->
      <el-dialog title="撤銷所有會話" :visible.sync="dialogVisibleCancelAllSession">
        <p>
          您是否確定要撤銷角色的當前所有會話？撤銷會話後，該角色當前所有訪問將立即被拒絕。
        </p>
      ...
      <rolePayloadRelation :visible.sync="rolePayloadRelationVisible" :relationList="currentRelationList"
        :relationPayload="currentRelationPayload" @saved="saveRelationList" />
    </div>
  </div>
</template>
<script>
  ...
  import rolePayloadRelation from "./component/rolePayloadRelation";
  ...
  export default {
    ...
    data() {
      return {
        ...
        consoleLogin: false,
        roleCarrierType: "",
        rolePayloadRelationVisible: false,
        currentRelationList: [],
        currentRelationPayload: "",
        PolicyDocument: ''
      };
    },
    mounted() {
    ...
    methods: {
      onChangeConsoleLogin(status) {
        this.axios
          .post(UPDATE_ROLE_CONSOLE_LOGIN, {
        ...
        this.axios
          ...
          .then(res => {
            if (res.Response.Error === undefined) {
              ...
              this.PolicyDocument = JSON.parse(resInfo.PolicyDocument)
              ...
              if (PolicyDocument.statement[0].principal.federated) {
                ...
                _this.roleCarrier = PolicyDocument.statement.map(s =>
                  typeof s.principal.federated === "string" ?
                  s.principal.federated :
                  s.principal.federated[0]
                );
          ...
          .then(res => {
            if (res.data.Error === undefined) {
              ...
              const index = res.data.list.findIndex(item =>
                item.policyName.includes("RevokeOlderSessionFor")
              );
              if (index !== -1) {
                this.axios
                  .post(GET_POLICY, {
                  ...
                  .then(({
                    ...
                    res.Response.list[index].validTime = moment(
                      JSON.parse(Response.PolicyDocument).statement[0].condition
                      .date_less_than["qcs:token_create_time"]
      ...
      delRolePolicy(index, rows, scope) {
        this.$confirm(this.$t("CAM.Role.jcjsztts"), this.$t("CAM.Role.jcjszt"), {
          confirmButtonText: "確定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.updateRolePolicy(index, rows, scope);
        });
      },
      // 修改角色信任策略
      updateRolePolicy(index, rows, scope) {
        if (this.$route.query.type == 'federated') {
          ...
          var policyDocument = {
            ...
            statement: [{
              ...
              effect: "allow",
              principal: {
                federated: this.roleCarrier
              }
            }]
          };
        } else {
          let bqcs = this.PolicyDocument.statement[0].principal.qcs
          let bservice = this.PolicyDocument.statement[0].principal.service
          let bprincipal = {}
          if (bqcs != undefined) {
            bqcs.forEach((item, index) => {
              if (item == scope) {
                bqcs.splice(index, 1)
              }
              return
            ...
            bprincipal.qcs = []
            bqcs.forEach(item => {
              bprincipal.qcs.push(item)
            });
          }
          if (bservice != undefined) {
            bservice.forEach((item, index) => {
              if (item == scope) {
                bservice.splice(index, 1)
              }
              return
            });
            bprincipal.service = []
            bservice.forEach(item => {
              bprincipal.service.push(item)
            });
          }
          var policyDocument = {
            version: "2.0",
      ...
      async cancelAllSession() {
        ...
        if (res.Response.TotalNum === 0) {
          res = await this.axios.post(CREATE_POLICY, {
            ...
            PolicyDocument: `{"version":"2.0","statement":[{"action":["*"],"resource":"*","effect":"deny","condition":{"date_less_than":{"qcs:token_create_time":"${moment()
            .utc()
            .format()}"}}}]}`
          });
      ...
      handleRolePayloadRelationVisible(row, relationList) {
        if (relationList === undefined) {
          relationList = [];
        }

        this.rolePayloadRelationVisible = true;
        this.currentRelationList = relationList;
        this.currentRelationPayload = row;
      },
      saveRelationList(policyDocument) {
        this.rolePayloadRelationVisible = false;
        ...
        this.axios.post(UPDATE_ASSUME, params).then(res => {
          if (res.Response.Error === undefined) {
            this.$message({
              message: "編輯條件成功",
              type: "success",
