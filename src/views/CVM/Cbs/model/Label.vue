  ...
  methods: {
    ...
    Label () {
      ...
      var parms = {
        Version: '2018-08-13',
        Region: this.$cookie.get('regionv2'),
        Resource: `qcs::cvm:${this.$cookie.get(
          'regionv2'
        )}:uin/100008405554:volume/${this.Cbsdata.DiskId}`
      }
      for (let i in this.keyValueArr) {
        parms['ReplaceTags.' + i + '.TagKey'] = this.keyValueArr[i].key
        parms['ReplaceTags.' + i + '.TagValue'] = this.keyValueArr[i].value
      }
      this.axios.post('tag2/ModifyResourceTags', parms).then(data => {
        console.log(data)
