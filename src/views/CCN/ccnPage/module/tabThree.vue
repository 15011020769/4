...
<template>
  <div class="tabThree">
    ...
    <div v-show="regionShow">
      <span>
        {{$t("CCN.total.tr7")}}：{{$t("CCN.tabs.tab3tit1")}}
        <span @click="updateBandwidthLimitTypeVisible = true"
          style="cursor: pointer;">{{$t("CCN.tabs.tab3tit2")}}</span>
      </span>
      <div class="table">
        <div class="btn">
        ...
        <el-table :data="tableData.slice((currentPage-1)*pageSize,currentPage*pageSize)" style="width: 100%"
          ...
          <el-table-column prop="CcnRegionBandwidthLimit.Region" :label="$t('CCN.tabs.tab3tr3')" width>
            <template slot-scope="scope">
              <p class="edit">{{Reginft[ scope.row.CcnRegionBandwidthLimit.Region] }}</p>
            </template>
          </el-table-column>
          <el-table-column prop="CcnRegionBandwidthLimit.DstRegion" :label="$t('CCN.tabs.tab3tr4')" width>
            <template slot-scope="scope">
              <p class="edit">{{ Reginft[scope.row.CcnRegionBandwidthLimit.DstRegion]}}</p>
            </template>
          </el-table-column>
          ...
          <table class="table-div">
            ...
            <td>
              <el-select v-model="upLimits.Region" :placeholder="$t('CCN.tabs.placeh1')">
                <el-option v-for="item in regionSet" :key="item.Region" :label="item.RegionName" :value="item.Region">
                </el-option>
              </el-select>
            </td>
            <td>
    ...
    <div v-show="!regionShow">
      <span>
        {{$t("CCN.total.tr7")}}：{{$t("CCN.tabs.tab3tit")}}
        <a @click="updateBandwidthLimitTypeVisible2 = true" style="cursor: pointer;">{{$t("CCN.tabs.tab3tit2")}}</a>
      </span>
      <div class="table">
        <div class="btn">
      ...
      <!-- 地域出 修改限速方式的模态窗 -->
      <el-dialog :title="$t('CCN.total.eWay')" :visible.sync="updateBandwidthLimitTypeVisible2" class="formDialog">
        <el-form :model="ccnPublic">
          <el-form-item :label="$t('CCN.total.eWay1')">
    ...
    data() {
      return {
        ...
        Reginft: {
          ...
          'ap-seoul': '亞太地區(首爾)',
          'ap-shanghai': ' 華東地區(上海)',
          'ap-shanghai-fsi': '華東地區(上海金融)',
          'ap-shenzhen-fsi': '華南地區(深圳金融)',
          'ap-singapore': '東南亞地區(新加坡)',
          'ap-tokyo': '亞太地區(東京)',
          'eu-frankfurt': '歐洲地區(法蘭克福)',
          'eu-moscow': ' 歐洲地區(莫斯科)',
          'na-ashburn': '美國東部(弗吉尼亞)',
          'na-siliconvalley': '美國西部(硅谷)',
          'na-toronto': ' 北美地區(多倫多)'
        }
      };
    },
