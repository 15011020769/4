    ...
    <div class="table">
      <el-table :data="tableData.slice((currentPage-1)*pageSize,currentPage*pageSize)" style="width: 100%"
        ...
        <el-table-column prop="operate" :label="$t('CCN.tabs.tab4tr6')" width>
          <template slot-scope="scope">
            <el-switch style="display: block;" v-model="scope.row.Enabled" active-color="#13ce66" inactive-color="#888"
              @click.native="switchClick(scope.$index, scope.row)"></el-switch>
          </template>
        </el-table-column>
      </el-table>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      switchClick: function (index, route) {
        let str = route.Enabled === false ? "關閉" : "啟用";
